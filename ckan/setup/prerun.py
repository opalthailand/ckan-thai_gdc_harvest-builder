...
def init_harvester():
    ...
    harvest_command = ['ckan', '-c', ckan_ini, 
                  'harvester', 'initdb']
    print('[prerun] Initializing or upgrading harvester - start')
    try:
        subprocess.check_output(harvest_command, stderr=subprocess.STDOUT)
        print('[prerun] Initializing or upgrading harvester - end')
    except subprocess.CalledProcessError as e:
        if 'OperationalError' in e.output:
            print(e.output)
            print('[prerun] Harvester not ready, waiting a bit before exit...')
            time.sleep(5)
            sys.exit(1)
        else:
            print(e.output)
